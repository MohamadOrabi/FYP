...
void Initialize() {
    ...
    float temp;
    int baud_rate = 9600;
    temp= (( (float) (8000000) / (float) baud_rate ) - 1);
    SPBRG = (int) temp;
...
void Forward(unsigned char chords){
    bool done = 0;
    bool S1A_old = PORTDbits.RD0;
    bool S1B_old = PORTDbits.RD1;
    ...
    while (!done) {
        ...
        if (PORTDbits.RD0 != S1A_old){
            xa++;
            S1A_old = PORTDbits.RD0;
        }

        if (PORTDbits.RD1 != S1B_old){
            xb++;
            S1B_old = PORTDbits.RD1;
        }
